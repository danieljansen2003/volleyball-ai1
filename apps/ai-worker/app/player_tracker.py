...
class PlayerTracker:
    """Memory-conscious two-pass player + volleyball tracker.

    This class is designed for small Render instances. It never keeps the
    player YOLO model and the custom volleyball YOLO model in memory at the
    same time. Player tracking runs first, the model is released, then the
    volleyball detector runs in a second pass over the video.
    """

    def __init__(self) -> None:
        worker_root = Path(__file__).resolve().parent.parent

        local_player_model = worker_root / "yolo11n.pt"
        self.player_model_source = (
            str(local_player_model)
            if local_player_model.exists()
            else "yolo11n.pt"
        )
        self.player_model_path = local_player_model

        self.ball_model_path = worker_root / "models" / "volleyball-ball.pt"
        ...
        # Keep this conservative on Render Free. Raise it locally later if the
        # custom detector needs more resolution.
        self.ball_imgsz = int(os.getenv("VV_BALL_IMGSZ", "640"))

        self.stitch_gap_seconds = float(
            os.getenv("VV_STITCH_GAP_SECONDS", "3.0")
        )
        self.stitch_distance = float(
            os.getenv("VV_STITCH_DISTANCE", "0.22")
        )

        print(
            "[player-tracker] Memory-sequential mode enabled. "
            "Player and volleyball models will be loaded one at a time.",
            flush=True,
        )
        print(
            f"[player-tracker] device={self.device} "
            f"tracker={tracker_path.name} "
            f"ball_model_ready={self.ball_model_path.exists()}",
            flush=True,
        )
    ...
    def _build_court_polygon(
        ...
        for point in court_points:
            ...
            if not 0 <= x <= 1 or not 0 <= y <= 1:
                raise ValueError(
                    "Court coordinates must be normalized between 0 and 1."
                )
            pixel_points.append(
    ...
    def _build_segments(
        ...
        for frame in raw_frames:
            ...
            for player in frame["players"]:
                ...
                segment["heights"].append(
                    float(player["box"]["y2"] - player["box"]["y1"])
                )
                segment["confidences"].append(
                    float(player["confidence"])
                )

        for segment in segments.values():
            heights = segment["heights"]
            segment["median_height"] = (
                float(np.median(heights)) if heights else 1.0
            )
    ...
    def _stitch_track_ids(
        ...
        ordered = sorted(
            segments.values(),
            key=lambda item: (item["start"], item["raw_id"]),
        )
        ...
        def position(
            ...
            foot = (
                segment["first_foot"]
                if first
                else segment["last_foot"]
            )
            return (
                float(foot["x"]) / max(1.0, width),
                float(foot["y"]) / max(1.0, height),
            )
        ...
        for segment in ordered:
            ...
            for stable_id, previous in stable_last.items():
                ...
                height_ratio = current_height / max(
                    1.0,
                    float(previous["median_height"]),
                )
                height_penalty = abs(
                    math.log(max(0.01, height_ratio))
                )

                if (
                    distance > self.stitch_distance
                    or height_penalty > 0.65
                ):
                    continue
    ...
    def _run_player_pass(
        ...
        court_polygon: np.ndarray | None,
        court_transform: np.ndarray | None,
        ...
        print("[player-tracker] Loading player model...", flush=True)
        player_model = YOLO(self.player_model_source)
        ...
        try:
            for frame_index, result in enumerate(results):
                ...
                if result.boxes is not None and len(result.boxes) > 0:
                    ...
                    for (
                        ...
                        track_frame_counts[raw_track_id] = (
                            track_frame_counts.get(raw_track_id, 0) + 1
                        )

                        court_position = self._project_point(
                            (foot_x, foot_y),
                            court_transform,
                        )
                ...
                if progress_callback and (
                    ...
                    progress_callback(
                        frame_index + 1,
                        max(1, reported_frame_count * 2),
                        f"Tracking players: frame {frame_index + 1} "
                        f"of {reported_frame_count}",
                    )
        finally:
            try:
                del result
            ...
            try:
                ...
                pass
            self._release_model(player_model)
            print(
                "[player-tracker] Player model released before ball pass.",
                flush=True,
            )
    ...
    def _run_ball_pass(
        ...
        if not self.ball_model_path.exists():
            print(
                f"[player-tracker] No custom volleyball detector at "
                f"{self.ball_model_path}; skipping ball pass.",
                flush=True,
            )
            return 0

        print(
            f"[player-tracker] Loading custom volleyball detector: "
            f"{self.ball_model_path}",
            flush=True,
        )
        ball_model = YOLO(str(self.ball_model_path))

        capture = cv2.VideoCapture(str(path))
        if not capture.isOpened():
            self._release_model(ball_model)
            raise RuntimeError(
                f"Could not reopen video for volleyball detection: {path}"
            )
        ...
        try:
            while True:
                ...
                if (
                    ...
                    ball_boxes = (
                        ball_results[0].boxes.xyxy.cpu().tolist()
                    )
                    ball_confidences = (
                        ball_results[0].boxes.conf.cpu().tolist()
                    )

                    candidates: list[dict[str, Any]] = []

                    for ball_box, ball_confidence in zip(
                        ball_boxes,
                        ball_confidences,
                        strict=True,
                    ):
                        bx1, by1, bx2, by2 = (
                            float(value) for value in ball_box
                        )
                        confidence = float(ball_confidence)
                        center_x = (bx1 + bx2) / 2.0
                        center_y = (by1 + by2) / 2.0

                        # Reject clearly impossible giant detections while
                        # remaining permissive for close-up volleyballs.
                        if (
                            (bx2 - bx1) > width * 0.15
                            or (by2 - by1) > height * 0.15
                        ):
                            continue
                ...
                if frame_index < len(raw_frames):
                    raw_frames[frame_index]["balls"] = balls

                # Do not retain prediction objects or source frames.
                del ball_results
                del frame
                ...
                if progress_callback and (
                    ...
                    progress_callback(
                        reported_frame_count + frame_index + 1,
                        max(1, reported_frame_count * 2),
                        f"Detecting volleyball: frame {frame_index + 1} "
                        f"of {reported_frame_count}",
                    )
            ...
            self._release_model(ball_model)
            print(
                "[player-tracker] Volleyball model released after ball pass.",
                flush=True,
            )
    ...
    def track(
        ...
        print(
            f"[player-tracker] video={path.name} "
        ...
        # At this point the player model has been fully destroyed and Python/
        # Torch caches have been cleared. Only then do we load the ball model.
        ball_detections = self._run_ball_pass(
            path=path,
        ...
        for frame in raw_frames:
            ...
            filtered_raw_frames.append(
                {**frame, "players": filtered_players}
            )

        id_mapping, stitching = self._stitch_track_ids(
            filtered_raw_frames,
            fps,
            width,
            height,
        ...
        for frame in filtered_raw_frames:
            ...
            for player in frame["players"]:
                stable_id = id_mapping.get(
                    int(player["raw_track_id"]),
                    int(player["raw_track_id"]),
                )
                ...
                stable_track_lengths[stable_id] = (
                    stable_track_lengths.get(stable_id, 0) + 1
                )
        ...
        print(
            ...
            f"raw_tracks={stitching['raw_track_count']} "
            f"stable_tracks={len(stable_ids)} "
            f"stitched={stitching['stitched_track_count']} "
            f"ball_detections={ball_detections}",
            flush=True,
        )
        ...
        return {
            ...
            "ball_model": (
                str(self.ball_model_path)
                if self.ball_model_path.exists()
                else None
            ),
            "ball_model_ready": self.ball_model_path.exists(),
            "memory_mode": "sequential_models",
            "ball_imgsz": self.ball_imgsz,
            "track_lengths": {
                str(k): v
                for k, v in sorted(stable_track_lengths.items())
            },
            "frames": frames,
        }